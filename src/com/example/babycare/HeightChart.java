...
public class HeightChart extends Activity {
	
	private GraphicalView lChart,sChart;
	private ChartDrawing lineChart,scatterChart;
    ...
    private Handler handler=new Handler(){
        ...
        public void handleMessage(Message msg){
            ...
            case 1:
            	...
            	if(IPAD.sex==1){
	        		for (int i = 0; i < length; i++) {
	        			userSD1[i] = userSD[i];
	        			minus3SD[i]=IPAD.height_boy[i][0];
	        			minus2SD[i]=IPAD.height_boy[i][1];
	        			minus1SD[i]=IPAD.height_boy[i][2];
	        			middleSD[i]=IPAD.height_boy[i][3];
	        			positive1SD[i]=IPAD.height_boy[i][4];
	        			positive2SD[i]=IPAD.height_boy[i][5];
	        			positive3SD[i]=IPAD.height_boy[i][6];
	        		}
        		}
            	else{
            		for (int i = 0; i < length; i++) {
	        			minus3SD[i]=IPAD.height_girl[i][0];
	        			minus2SD[i]=IPAD.height_girl[i][1];
	        			minus1SD[i]=IPAD.height_girl[i][2];
	        			middleSD[i]=IPAD.height_girl[i][3];
	        			positive1SD[i]=IPAD.height_girl[i][4];
	        			positive2SD[i]=IPAD.height_girl[i][5];
	        			positive3SD[i]=IPAD.height_girl[i][6];
	        		}
            	}
            	 //double[] income = { 1000,3700,3800,2000,2500,6000,1000,8000,9000,5000,2000};
                lineChart=new ChartDrawing("月龄","身高（长）\\cm","Test Chart",mMonth);
           ...
           //	sChart = (GraphicalView) ChartFactory.getScatterChartView(getBaseContext(), lineChart.getDataset_scatter(), lineChart.getMultiRenderer_scatter());	
           		// Adding the Line Chart to the LinearLayout
           		lineChartContainer.addView(lChart);
           //		lineChartContainer.addView(sChart);
                break;
            }
        }
    };
    ...
    private void geneItems(String user) {
       ...
       try{
            ...
            HttpPost httpget = new HttpPost("http://"+IPAD.ip+"/babycare/getheight.php");                
       ...
       try{
            ...
            for(int i=0;i<jArray.length();i++){
           	  ...
           	  userSD[i]=json_data.getDouble("height");
           	  if(userSD[i]>0)
           		  length = i+1;
